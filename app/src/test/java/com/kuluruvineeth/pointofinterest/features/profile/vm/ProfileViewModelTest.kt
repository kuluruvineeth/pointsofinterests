package com.kuluruvineeth.pointofinterest.features.profile.vm
...
import com.kuluruvineeth.pointofinterest.MockitoHelper.anyNonNull
import com.kuluruvineeth.pointofinterest.MockitoHelper.mock
import com.kuluruvineeth.pointofinterest.MockitoHelper.whenever
import com.kuluruvineeth.pointofinterest.features.profile.models.ProfileScreenUiState
import com.kuluruvineeth.pointofinterest.features.profile.models.ProfileSectionItem
import com.kuluruvineeth.pointofinterest.features.profile.models.ProfileSectionType
import com.kuluruvineeth.pointofinterest.features.profile.models.wipSections
...
class ProfileViewModelTest {
    ...
    fun `test ProfileViewModel onSettingsToggled SYSTEM_THEME updates profileState with updated user settings data`() = runTest {
        ...
        assertIs<ProfileScreenUiState.Result>(state)
        assertEquals(true, state.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.SYSTEM_THEME)?.state)
        assertEquals(false, state.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.DARK_THEME)?.isEnabled)
        ...
        assertEquals(false, updated.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.SYSTEM_THEME)?.state)
        assertEquals(true, updated.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.DARK_THEME)?.isEnabled)
        collectJob.cancel()
    }
    ...
    fun `test ProfileViewModel onSettingsToggled DARK_THEME updates profileState with updated user settings data`() = runTest {
        ...
        assertIs<ProfileScreenUiState.Result>(state)
        assertEquals(false, state.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.DARK_THEME)?.state)
    ...
    fun `test ProfileViewModel onSettingsToggled GARBAGE_COLLECTOR updates profileState with updated user settings and triggers work manager`() = runTest {
        ...
        assertIs<ProfileScreenUiState.Result>(state)
        assertEquals(false, state.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.GARBAGE_COLLECTOR)?.state)
        ...
        assertEquals(false, updated2.findSection<ProfileSectionItem.BooleanSettingsItem>(
            ProfileSectionType.GARBAGE_COLLECTOR)?.state)
