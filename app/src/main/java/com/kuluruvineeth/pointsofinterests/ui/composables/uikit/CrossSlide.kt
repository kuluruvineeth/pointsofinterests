...
fun <T> CrossSlide(
    ...
    val items = remember {
        mutableStateListOf<SlideInOutAnimationState<T>>()
    }
    ...
    val transition: Transition<MutableTransitionState<T>> = updateTransition(targetState = transitionState, label = "")
    ...
    if(targetChanged || items.isEmpty()){
                ...
                Box(modifier = modifier.graphicsLayer {
                    this.translationX =
                        if(transition.targetState == key)
                            direction * xTransition.x else direction * -xTransition.x
                }){
                    content(key)
                }
            }
        ...
        items.removeAll{
            it.key != transitionState.targetState
        }
    }
