package com.kuluruvineeth.pointsofinterests.features.profile.models
...
sealed class ProfileSectionItem(
    ...
    data class AccountSectionItem(val userInfo: UserInfo?):
            ProfileSectionItem(ProfileSectionType.ACCOUNT)
...
fun ProfileSectionType.toSubTitle(): Int? =
    when(this){
        ProfileSectionType.SYSTEM_THEME ->
            R.string.profile_section_subtitle_system_theme
        ProfileSectionType.GARBAGE_COLLECTOR ->
            R.string.profile_section_subtitle_gc
        else -> null
    }
