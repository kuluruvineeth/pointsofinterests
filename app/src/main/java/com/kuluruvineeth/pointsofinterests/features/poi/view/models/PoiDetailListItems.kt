package com.kuluruvineeth.pointsofinterests.features.poi.view.models
...
import com.kuluruvineeth.pointsofinterests.core.utils.CommonFormats
...
sealed class PoiDetailListItem(val uniqueKey: String){
    data class ImageItem(val imageUrl: String):
            PoiDetailListItem(ImageItem::class.java.simpleName)
    data class MetadataItem(val dateTime: String):
            PoiDetailListItem(MetadataItem::class.java.simpleName)
    data class CategoriesItem(val categoryUiModel: List<CategoryUiModel>):
            PoiDetailListItem(CategoriesItem::class.java.simpleName)
    data class TitleItem(val text: String):
            PoiDetailListItem(TitleItem::class.java.simpleName)
    data class BodyItem(val text: String):
            PoiDetailListItem(BodyItem::class.java.simpleName)
    data class ContentUrl(val source: String, val url: String):
            PoiDetailListItem(ContentUrl::class.java.simpleName)
    data class CommentsCount(val count: Int):
            PoiDetailListItem(CommentsCount::class.java.simpleName)
...
fun PoiModel.toUIModelWithComments(comments: List<PoiComment>): List<PoiDetailListItem>{
    ...
    imageUrl?.takeIf { it.isNotEmpty() }?.let { url -> result +=
        PoiDetailListItem.ImageItem(url)
    }
    ...
    body?.takeIf { it.isNotEmpty() }?.let { body -> result +=
        PoiDetailListItem.BodyItem(body)
    }
    contentLink?.takeIf { it.isNotEmpty() }
        ?.let { contentLink -> result +=
            PoiDetailListItem.ContentUrl(requireNotNull(source),contentLink)
        }
