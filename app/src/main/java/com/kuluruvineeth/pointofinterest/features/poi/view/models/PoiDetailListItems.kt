package com.kuluruvineeth.pointofinterest.features.poi.view.models
...
import com.kuluruvineeth.pointofinterest.core.utils.CommonFormats
...
sealed class PoiDetailListItem(val uniqueKey: String) {
    ...
    data class CategoriesItem(val categoryUiModel: List<CategoryUiModel>) : PoiDetailListItem(
        CategoriesItem::class.java.simpleName)
...
fun PoiModel.toUIModelWithComments(comments: List<PoiComment>): List<PoiDetailListItem> {
    ...
    contentLink?.takeIf { it.isNotEmpty() }
        ?.let { contentLink -> result += PoiDetailListItem.ContentUrl(
