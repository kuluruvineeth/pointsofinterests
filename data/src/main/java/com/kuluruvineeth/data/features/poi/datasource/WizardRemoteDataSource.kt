...
class WizardRemoteDataSource @Inject constructor(
    ...
    private fun MediaType?.isImageContentType() = this?.type ==
            CONTENT_TYPE_IMAGE || this?.type == CONTENT_TYPE_BINARY


    private fun Document.toSuggestion(originalUrl: String):
            WizardSuggestionDataModel{
        ...
        var image: String? =
            select("meta[property=og:image]").attr("content").resolveUrl(originalUrl)
